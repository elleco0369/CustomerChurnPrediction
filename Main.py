...
df['Total Charges'] = df['Total Charges'].fillna(df['calc_charges'])
df.drop(columns=['calc_charges'], inplace=True)
...
# === 3. Encoding & Split ===
X = pd.get_dummies(df.drop(columns=["churn_value"]), drop_first=True)
y = df["churn_value"]
...
metrics = {
    "RF + FI":   get_metrics(y_test, rf_fi.predict(X_test_fi)),
    "RF + RFE":  get_metrics(y_test, rf_rfe.predict(X_test_rfe)),
    "XGB + FI":  get_metrics(y_test, xgb_fi.predict(X_test_fi)),
    "XGB + RFE": get_metrics(y_test, xgb_rfe.predict(X_test_rfe))}
ax = pd.DataFrame(metrics).plot(
    kind="bar",
    figsize=(12, 8),
    edgecolor="black",
    width=0.8,
...
ax.yaxis.grid(True, which='major', linestyle='--', linewidth=0.8, alpha=0.7, zorder=3)
ax.yaxis.grid(True, which='minor', linestyle=':', linewidth=0.5, alpha=0.4, zorder=3)
plt.tight_layout()
plt.show()